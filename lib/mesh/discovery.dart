...
///
/// Multicast is the primary path. Android also uses IPv4 broadcast as a
/// fallback, because some access points do not forward multicast reliably.
class DiscoveryService {
  ...
  Future<void> start() async {
    ...
    // Required before IPv4 broadcast sends on Android/Dart VM.
    if (canBroadcast) {
      try {
  ...
  Future<void> _joinGroups() async {
    ...
    try {
      for (final iface in await NetworkInterface.list()) {
        if (!iface.addresses.any((a) => a.type == InternetAddressType.IPv4)) {
          continue;
        }
        try {
