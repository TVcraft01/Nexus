...
/// A clean presentation layer for the existing assistant engines.
///
/// The important distinction is intentional: this screen is a human-facing
/// conversation surface. It does not expose the command catalog as the product
/// itself. The command service, brain, executor and mesh remain underneath it.
...
class _NexusV2AssistantViewState extends State<NexusV2AssistantView> {
  ...
  void initState() {
    ...
    _service = CommandService(
      ...
      onPhraseLearned: (phrase, meaning) => unawaited(widget.mesh.broadcastLearnedPhrase(phrase, meaning)),
      onFactLearned: (fact) => unawaited(widget.mesh.broadcastFact(fact)),
      onDefaultLearned: (key, value) => unawaited(widget.mesh.broadcastDefault(key, value)),
    );
  ...
  Future<void> _submit({bool voice = false}) async {
    ...
    final result = _service.execute(
      ...
      approval: _pendingApproval == text ? AgentApproval.approved : AgentApproval.required,
    );
    ...
    switch (result.dispatch) {
      ...
      case final AgentMessage message:
        ...
        if (message.action == null && widget.brain != null) {
          // Let the conversational engine take over only for messages that
          // are not already actionable.
  ...
  Future<void> _conversationFallback(String text, AgentDispatchResult result) async {
  ...
  Future<void> _dispatchPlan(AgentActionPlan plan, {required bool spoken}) async {
    if (plan.request.target == widget.mesh.identity.id && _selfRunActions.contains(plan.request.action)) {
      await _runLocal(plan.request, spoken: spoken);
      return;
    }
    if (plan.request.target.isNotEmpty) {
      ...
      _conversation.appendResult(
        AgentDispatchResult(
          ...
          message: reply == null ? 'I could not reach that device.' : _describe(reply),
  ...
  Future<void> _listen() async {
    ...
    if (!speech.available) {
      _conversation.appendResult(const AgentDispatchResult(dispatch: AgentMessage('Voice input is not available on this device yet.')));
      if (mounted) setState(() {});
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      ...
      child: Column(
        children: [
          NexusV2PageTitle(
            title: _profileLoaded ? (_profileState?.assistantName ?? 'Nexus') : 'Nexus',
            subtitle: _conversation.isEmpty ? 'Ready when you are.' : 'Here to help.',
            trailing: IconButton(
          ...
          Expanded(
            child: entries.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) => _message(entries[index], theme),
                  ),
          ),
          if (_pendingApproval != null) _ApprovalBar(onApprove: () => _approvePending(), onDismiss: () => setState(() => _pendingApproval = null)),
          _composer(theme),
        ],
      ),
    );
  ...
  Widget _welcome(ThemeData theme) => Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Icon(Icons.auto_awesome_rounded, size: 38, color: theme.colorScheme.primary),
              const SizedBox(height: 18),
              Text(_greeting, style: theme.textTheme.headlineMedium, textAlign: TextAlign.center),
              const SizedBox(height: 8),
              Text(
                'Ask naturally. I can work with this device, your connected devices and the tools you have enabled.',
    ...
    return Align(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (entry.status != null && entry.status != AgentResultStatus.succeeded) ...[
            const SizedBox(height: 5),
            Text(entry.status!.name, style: theme.textTheme.bodySmall?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
          ],
        ]),
      ),
    );
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              IconButton(
                ...
                color: _listening ? theme.colorScheme.primary : theme.colorScheme.onSurfaceVariant,
              ),
              IconButton.filled(
                tooltip: 'Send',
        ...
        child: Card(
          child: Padding(
            ...
            child: Row(children: [
              ...
              FilledButton(onPressed: onApprove, child: const Text('Allow')),
            ]),
          ),
        ),
      );
