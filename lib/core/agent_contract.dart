...
abstract final class AgentActions {
  ...
  static const profileSet = 'profile.set';
...
/// What a status means, in words. The vocabulary lives with the enum it
/// describes so the three places that speak about a result — the chip, the
/// line under "Sent to …", and the fallback when a result carries no reason
/// — cannot drift apart or disagree about what the same status means.
///
...
extension AgentResultStatusWording on AgentResultStatus {
  ...
  String get label => switch (this) {
    ...
    AgentResultStatus.denied => 'Denied',
  ...
  ///
  /// A bare verdict like "Unavailable" tells the user nothing about what is
  /// missing or what would change it, and reads as the app pretending it
  /// tried. Results normally carry a specific reason, but one can arrive
  /// empty — a few dispatch paths return none, and a paired device can send
  /// one over the mesh — so a non-success status always falls back to
  /// explaining itself. Success has nothing to apologise for and stays empty.
  String explain([String message = '']) {
