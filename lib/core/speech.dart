...
///
/// The speak call above resolves when an utterance is *queued*, so on its own
/// it only proves the engine accepted the text — not that anything is being
/// said. Android's text-to-speech engine reports the difference itself: it
/// announces when an utterance starts and when it finishes or is
/// interrupted, and the platform half forwards those over
/// `dev.nexus.nexus/speech_state`. That makes [speaking] a fact about this
/// instant rather than an optimistic guess, which is the only reason the
/// assistant's globe is allowed to show a speaking state at all.
///
/// Where the platform cannot report it (everywhere but Android today),
/// [available] is false and [speaking] stays false: the globe then never
/// claims a voice is playing, which is the honest answer.
class SpeechPlayback {
  SpeechPlayback();

  static SpeechPlayback? _instance;

  /// Test seam, like the other backends: a fake playback source, so "the
  /// globe showed speaking while Nexus spoke" is provable without a speaker.
  @visibleForTesting
  ...
  /// implementation to activate), which the test framework counts as a
  /// failure rather than something a stream's own error handler can absorb.
  bool get available =>
